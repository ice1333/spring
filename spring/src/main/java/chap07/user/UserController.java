...
public class UserController {
	...
	public String regist2(Model model, UserVo vo, HttpServletRequest req) {
		System.out.println("등록전 userno:"+vo.getUserno());
		boolean r =service.insert(vo, req);  //변형이 일어난 개수를 r에
	...
	public String login(@CookieValue(value="cookieId", required=false) Cookie cookie, UserVo vo) {
		if(cookie != null) { //이전에 아이디 저장체크하고 로그인한 경우
...
//			vo.setCheckId("check");
	...
	public String loginProcess(UserVo vo, HttpSession sess, Model model , HttpServletResponse res) {
		...
		if(service.login(vo, sess)) { //매서드의 리턴값이들어감 조회후 세션에저장
			//쿠키객체 생성
			Cookie cookie = new Cookie("cookieId", vo.getId());
			if("check".equals(vo.getCheckId())) { //사용자가 아이디저장체크
				cookie.setMaxAge(60*60*24*30);//초단위부터 시작 60초 1시간 하루 30일
			} else {
				cookie.setMaxAge(0);//유효기간을 0으로 해서 없앰 즉시만료
...
//	@RequestMapping("/user/mypage.do")//다른컨트롤러에있음
